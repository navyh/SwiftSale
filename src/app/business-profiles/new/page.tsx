...
const profileFormSchema = z.object({
  ...
  paymentTerms: z.string().optional().nullable().default("NET 30"),
  creditLimit: z.coerce.number({invalid_type_error: "Credit limit must be a number"}).min(0, "Credit limit cannot be negative").optional().nullable().default(30),
  ...
  React.useEffect(() => {
    async function loadUsers() {
      ...
      try {
        ...
        // Filter for customer users (those with no specific role or role explicitly set to null/undefined)
        setAllUsers(users.filter(u => u.role === null || u.role === undefined));
      } catch (error: any) {
        toast({
  ...
  async function onSubmit(data: ProfileFormValues) {
    ...
    const businessProfilePayload = {
      ...
      addresses: data.addresses.map(addr => ({
        ...
        city: addr.city, // already required
        state: addr.state, // already required
        stateCode: addr.stateCode, // already required
      }))
    };
    ...
    try {
      if (data.creationMode === "linkExistingUser") {
        ...
        const apiPayload: CreateBusinessProfileRequest = {
            ...
            status: businessProfilePayload.status,
            addresses: businessProfilePayload.addresses,
        };
        await createBusinessProfile(apiPayload, data.creatorUserId);
      } else { // createNewUser mode
        const userAddresses: AddressCreateDto[] = data.addresses.length > 0 ? [{
          ...data.addresses[0],
          type: data.addresses[0].type || "BILLING",
          isDefault: true, // User's first address should be default
      ...
      console.error("Error during business profile submission:", error);
      toast({
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 md:space-y-8">
                    ...
                    <FormControl>
                      <RadioGroup
                        onValueChange={(value) => {
                          ...
                          // Optionally reset fields when mode changes
                          if (value === "linkExistingUser") {
                            form.setValue("newUserName", "");
                            form.setValue("newUserPhone", "");
                            form.setValue("newUserEmail", "");
                          } else {
                            form.setValue("creatorUserId", undefined);
                          }
                        }}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        <FormControl><SelectTrigger><SelectValue placeholder={isLoadingUsers ? "Loading users..." : "Select a customer user"} /></SelectTrigger></FormControl>
                        <SelectContent>
                          {isLoadingUsers ? (
          ...
          <Card className="shadow-md">
            ...
            <CardContent className="space-y-4">
                      ...
                      render={({ field: stateField }) => (
                        <FormItem>
                          ...
                          <StateCombobox
                            ...
                            onStateCodeChange={() => { /* Not directly used here, stateCode set in onValueChange */ }}
                          />
              ...
              <Button type="button" variant="outline" onClick={() => appendAddress({ line1: '', line2: '', city: '', state: '', stateCode: '', country: 'India', postalCode: '', isDefault: addressFields.length === 0, type: 'BILLING' })}>
